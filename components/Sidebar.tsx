  ...
  return (
    <>
      ...
      <aside className={`fixed inset-y-0 left-0 z-[70] bg-white border-r border-slate-100 transition-all duration-500 md:relative md:translate-x-0 ${isOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'} ${isCollapsed ? 'md:w-[90px]' : 'lg:w-[280px] md:w-[240px]'} flex flex-col shadow-2xl md:shadow-none`}>
        ...
        {/* Logo Section - Updated to match your Logo Image */}
          ...
          <Link to={AppRoutes.HOME} className="flex items-center gap-4">
            <div className="w-16 h-16 bg-theme-surface rounded-[1.5rem] flex items-center justify-center shrink-0 shadow-lg shadow-black/10 overflow-hidden group">
              <div className="w-10 h-10 bg-white rounded-lg flex items-center justify-center p-1.5 transition-transform group-hover:scale-110">
                <img src="https://res.cloudinary.com/dazlddxht/image/upload/v1768234409/SS_Paw_Pal_Logo_aceyn8.png" className="w-full h-full object-contain" />
              </div>
            </div>
            {!isCollapsed && (
        ...
        {/* Navigation - Updated to match your Dashboard Button Image */}
          ...
          {!isCollapsed && user && (
            <div className="px-4 py-3 bg-slate-50 rounded-2xl flex items-center gap-3">
                <div className="w-8 h-8 rounded-lg bg-white overflow-hidden border border-slate-100 shrink-0">
                    <img src={user.photoURL || `https://ui-avatars.com/api/?name=${user.displayName || user.email?.split('@')[0]}`} className="w-full h-full object-cover" />
                </div>
                <div className="min-w-0">
                    <p className="text-[10px] font-black text-slate-800 truncate uppercase tracking-widest leading-none">{user.displayName || 'Guardian'}</p>
                    <p className="text-[8px] font-bold text-slate-400 truncate mt-1">{user.email}</p>
                </div>
            </div>
          )}
