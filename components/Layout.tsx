...
const Layout: React.FC<LayoutProps> = ({ children }) => {
  ...
  return (
    <div className="min-h-screen w-full bg-slate-200/50 flex items-center justify-center py-0 md:py-8 px-0 md:px-4 overflow-hidden">
      {/* 80% App Ratio Wrapper for Desktop */}
      <div className="flex h-[100vh] md:h-[90vh] w-full md:w-[92%] lg:w-[80%] overflow-hidden bg-slate-50 md:rounded-[3.5rem] md:shadow-[0_50px_100px_-20px_rgba(0,0,0,0.2)] md:border md:border-white/50 backdrop-blur-sm">
        ...
        <Sidebar 
          ...
          // Fixed: Use setIsSidebarCollapsed instead of setIsCollapsed
        ...
        <div className="flex-1 flex flex-col overflow-hidden relative">
          {/* Top Header */}
          <header className="h-20 bg-white/70 backdrop-blur-md border-b border-slate-100 flex items-center justify-between px-4 md:px-10 z-30 shadow-sm">
            <div className="flex items-center gap-4">
              <button 
                onClick={() => setIsSidebarOpen(true)}
                className="md:hidden p-2.5 text-slate-600 hover:bg-slate-100 rounded-xl transition-all active:scale-90"
            ...
            <div className="flex items-center gap-2 md:gap-5">
              ...
              <button className="flex items-center gap-3 p-1.5 pr-4 hover:bg-slate-50 rounded-2xl transition-all group border border-transparent hover:border-slate-100">
                ...
                <div className="hidden md:block text-left">
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none">Parent</p>
          ...
          <main className="flex-1 overflow-y-auto p-4 md:p-12 custom-scrollbar bg-slate-50/30">
