...
const AVATAR_STYLES = [
  ...
  { id: 'pixel-art', name: 'Retro Pixel', description: 'Nostalgic 16-bit video game art', prompt: 'A vibrant 16-bit pixel art pet portrait. Nostalgic video game aesthetic, clean grid-aligned pixels, limited retro color palette, crisp edges, simplified shapes, high-contrast shading.' },
  { id: 'cyberpunk', name: 'Cyber Neon', description: 'Futuristic neon & high-tech glow', prompt: 'A gritty, futuristic cyberpunk pet portrait. Neon-drenched lighting, high-tech cyber-implants, rain-slicked city streets in background, pink and cyan color grading, dramatic shadows.' },
...
const PetProfilePage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const loadPetsData = async () => {
      ...
      } catch (err) { console.warn("Cloud sync failed, using local registry:", err); }
      finally { setIsSyncing(false); }
    };
    loadPetsData();
  }, [user?.uid]);

  // Handle generating AI age milestones
  const generateAgeMilestones = async () => {
    if (!selectedPet) return;
    setIsGeneratingMilestones(true);
    try {
      const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });
      const prompt = `Provide concise, expert developmental milestones and care advice for a ${selectedPet.species} (${selectedPet.breed}) who is ${selectedPet.ageYears} years and ${selectedPet.ageMonths} months old. Focus on physical development, behavioral changes, and specific diet/play needs for this exact life stage. Keep it under 100 words in a bulleted list format.`;
      const response = await ai.models.generateContent({ model: 'gemini-3-flash-preview', contents: prompt });
      setAgeMilestones(response.text || "No specific milestones available for this stage.");
    } catch (err) {
      addNotification('AI Error', 'Failed to fetch life stage intelligence.', 'error');
    } finally {
      setIsGeneratingMilestones(false);
    }
  };

  // Trigger milestones whenever pet changes
  useEffect(() => {
    if (selectedPet) {
      setAgeMilestones(null);
      generateAgeMilestones();
    }
  }, [selectedPet?.id, selectedPet?.birthday]);
  ...
  const handleDeletePet = async () => {
    ...
    try {
      ...
      setIsAdding(false); 
    } catch (err) { addNotification('System Error', 'Failed to remove pet from registry.', 'error'); }
  ...
  const generateHealthInsights = async () => {
    ...
    try {
      const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });
      const prompt = `Analyze the following pet data and provide health insights and recommendations: Pet Name: ${selectedPet.name}, Species: ${selectedPet.species}, Breed: ${selectedPet.breed}, Age: ${selectedPet.ageYears}y ${selectedPet.ageMonths}m, Weight History: ${JSON.stringify(selectedPet.weightHistory)}, Vaccinations: ${JSON.stringify(selectedPet.vaccinations)}. Provide 3-4 bullet points of high-level advice focusing on weight trends, vaccination status, and general wellness. Keep it under 100 words.`;
      const response = await ai.models.generateContent({ model: 'gemini-3-flash-preview', contents: prompt });
    ...
    } catch (err) { addNotification('AI Error', 'Failed to generate health insights.', 'error'); }
  ...
  const handleDeleteRecord = async (type: 'vaccine' | 'weight', index: number) => {
    if (!selectedPet) return;
    let updatedPet = { ...selectedPet };
    if (type === 'vaccine') { updatedPet.vaccinations.splice(index, 1); }
    else { updatedPet.weightHistory.splice(index, 1); }
    const updatedPets = pets.map(p => p.id === selectedPet.id ? updatedPet : p);
    await savePetsToStorage(updatedPets);
    setSelectedPet(updatedPet);
  ...
  const generateAIAvatar = async (styleId: string, base64Source?: string) => {
    ...
    } catch (err: any) { addNotification('AI Studio Error', 'Generation failed.', 'error'); }
    finally { setIsGeneratingAvatar(false); }
  };

  const handleScanClick = () => qrFileInputRef.current?.click();
  ...
  const handleFileScan = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      ...
      img.onload = async () => {
        ...
        if (code) {
          const petId = code.data.split('/').pop() || code.data;
          try {
            const petData = await getPetById(petId);
            if (petData) {
              addNotification('ID Verified', `Found registry for ${petData.name}.`, 'success');
              const userPet = pets.find(p => p.id === petData.id);
              if (userPet) { setSelectedPet(userPet); setIsAdding(false); }
              else { navigate(`/pet/${petData.id}`); }
            } else { addNotification('Registry Missing', 'Invalid QR signature.', 'warning'); }
          } catch (err) { addNotification('Network Failure', 'Could not verify.', 'error'); }
        } else { addNotification('Scan Interrupted', 'No valid digital signature found.', 'error'); }
        setIsScanning(false);
      };
        ...
        <>
          ...
          {isAdding ? (
                ...
                <div className="bg-white rounded-[2.5rem] p-8 border border-slate-100 shadow-xl text-center space-y-6 relative overflow-hidden group">
                  ...
                  <div className="flex gap-2 justify-center relative z-10">
                    <button onClick={() => fileInputRef.current?.click()} className="p-3.5 bg-white border border-slate-100 rounded-xl hover:bg-slate-50 shadow-sm text-slate-500 transition-all hover:text-theme" title="Upload Reference Photo">
                      <Camera size={20} />
                      <input type="file" ref={fileInputRef} className="hidden" accept="image/*" onChange={(e) => { const file = e.target.files?.[0]; if (file) { const reader = new FileReader(); reader.onloadend = () => generateAIAvatar(selectedPet.avatarStylePreference || AVATAR_STYLES[0].id, reader.result as string); reader.readAsDataURL(file); } }} />
                    </button>
                    <button onClick={() => setShowStyleModal(true)} disabled={isGeneratingAvatar} className="p-3.5 rounded-xl shadow-lg transition-all bg-slate-900 text-theme hover:bg-black flex items-center gap-2" title="Open AI Portrait Studio">
                      <Wand2 size={20} />
                  ...
                  <div className="space-y-1 pb-4 relative z-20">
                    ...
                    {selectedPet.avatarStylePreference && (
                      <div className="mt-3 inline-flex items-center gap-2 px-3 py-1 bg-slate-50 border border-slate-100 rounded-full">
                        <Palette size={12} className="text-slate-400" />
                        <span className="text-[9px] font-black uppercase tracking-widest text-slate-400">Style: {AVATAR_STYLES.find(s => s.id === selectedPet.avatarStylePreference)?.name}</span>
                      </div>
                    )}

                    <button onClick={() => setShowDeleteModal(true)} className="mt-6 flex items-center gap-2 mx-auto text-rose-400 hover:text-rose-600 font-bold text-[10px] uppercase tracking-widest transition-colors cursor-pointer">
                ...
                <div className="bg-slate-900 rounded-[2.5rem] p-8 text-white space-y-6 shadow-2xl relative overflow-hidden">
                   ...
                   <div className="space-y-4 pt-2 relative z-10">
                      ...
                      <button 
                        ...
                        className="w-full flex items-center justify-center gap-3 py-4 bg-indigo-600 text-white rounded-xl font-black text-xs uppercase tracking-widest hover:bg-indigo-700 transition-all shadow-lg active:scale-95"
                      >
                        <Download size={18} /> Share Digital ID
                      </button>
                      <button onClick={() => navigate(`/pet/${selectedPet.id}`)} className="w-full flex items-center justify-center gap-2 py-3 bg-white/5 text-white rounded-xl font-black text-[9px] uppercase tracking-widest hover:bg-white/10 transition-all">
              ...
              <div className="lg:col-span-2 space-y-8">
                ...
                <div className="bg-indigo-600 rounded-[2.5rem] p-8 text-white relative overflow-hidden group">
                  ...
                  <div className="relative z-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
                    <div className="flex items-center gap-4">
                      <div className="p-4 bg-white/10 backdrop-blur-md rounded-[1.5rem] border border-white/20">
                        <Calendar size={28}/>
                      </div>
                      <div>
                        <h4 className="text-2xl font-black tracking-tight">Life Phase Intelligence</h4>
                        <p className="text-indigo-100/70 font-black text-[10px] uppercase tracking-widest mt-1">Current Milestone: {selectedPet.ageYears}y {selectedPet.ageMonths}m</p>
                      </div>
                    </div>
                  </div>
                  <div className="mt-8 relative z-10">
                    {isGeneratingMilestones ? (
                      <div className="flex items-center gap-3 py-4 text-indigo-100/50 italic animate-pulse">
                        <Loader2 size={16} className="animate-spin" />
                        <span className="text-sm font-bold uppercase tracking-widest">Analyzing development data...</span>
                      </div>
                    ) : ageMilestones ? (
                      <div className="bg-white/10 backdrop-blur-sm rounded-[2rem] p-6 border border-white/10 animate-in fade-in slide-in-from-bottom-2">
                        <div className="prose prose-invert prose-sm max-w-none">
                          <p className="text-white text-sm leading-relaxed whitespace-pre-line font-medium">{ageMilestones}</p>
                        </div>
                      </div>
                    ) : (
                      <div className="py-4 text-indigo-100/40 italic text-sm font-bold">Could not load stage insights.</div>
                    )}
                  </div>
                </div>
                ...
                <div className="bg-white rounded-[2.5rem] p-8 border border-slate-50 shadow-sm flex flex-col">
                  <div className="flex items-center gap-4 mb-6">
                    <div className="p-3 bg-amber-50 text-amber-600 rounded-xl"><Smile size={24} /></div>
                    <div>
                      <h4 className="font-black text-xl text-slate-900 leading-none">Temperament</h4>
                      <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mt-1">Personality & Habits</p>
                    </div>
                  </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        {isAddingRecord === 'vaccine' ? (
                          <>
                            ...
                            <div className="space-y-1 md:col-span-2 animate-in fade-in slide-in-from-top-1">
                ...
                <div className="bg-slate-900 rounded-[2.5rem] p-8 border border-slate-800 shadow-xl overflow-hidden relative group">
                   ...
                   <div className="flex items-center justify-between mb-8 relative z-10">
                      ...
                      <button onClick={generateHealthInsights} disabled={isAnalyzingHealth} className="px-6 py-2.5 bg-theme text-white rounded-xl text-[9px] font-black uppercase tracking-widest flex items-center gap-2 hover:bg-theme-hover transition-all active:scale-95 disabled:opacity-50">
            ...
            <div className="py-40 text-center animate-in zoom-in-95 duration-500">
              ...
              <button onClick={() => { setStep(1); setIsAdding(true); setFormErrors({}); }} className="bg-slate-900 text-white px-10 py-5 rounded-[1.5rem] font-black text-xs uppercase tracking-widest hover:bg-black transition-all shadow-2xl shadow-slate-200 active:scale-95">
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-slate-900/60 backdrop-blur-md animate-in fade-in">
          <div className="bg-white rounded-[3.5rem] w-full max-w-5xl overflow-hidden shadow-2xl animate-in zoom-in-95 duration-300 flex flex-col max-h-[90vh]">
              ...
              <div className="flex items-center gap-6">
                ...
                <div>
                  <h3 className="text-3xl font-black text-slate-900 tracking-tight flex items-center gap-3">
                    Portrait Studio
                    <span className="text-[10px] font-black uppercase tracking-widest bg-indigo-50 text-indigo-600 px-3 py-1 rounded-full border border-indigo-100">AI Artist Suite</span>
                  </h3>
              ...
              {AVATAR_STYLES.map(style => (
                <button key={style.id} onClick={() => generateAIAvatar(style.id)} className={`group relative flex flex-col p-8 rounded-[2.5rem] border transition-all text-left overflow-hidden h-full ${style.isPremium ? 'bg-slate-900 border-slate-800 text-white ring-8 ring-indigo-500/5' : 'bg-white border-slate-100 hover:border-theme hover:shadow-2xl hover:-translate-y-1'}`}>
                  <div className="flex items-center justify-between mb-6 relative z-10">
                    <div className={`p-3 rounded-2xl ${style.isPremium ? 'bg-indigo-500/20 text-indigo-400' : 'bg-slate-50 text-slate-400 group-hover:bg-theme-light group-hover:text-theme'}`}>{style.isPremium ? <Sparkles size={20} /> : <Palette size={20} />}</div>
                    {style.isPremium && <div className="flex items-center gap-2 px-3 py-1 bg-indigo-500/10 rounded-full"><Star size={12} className="text-indigo-400 fill-indigo-400" /><span className="text-[8px] font-black uppercase tracking-[0.2em] text-indigo-400">Elite Model</span></div>}
                  </div>
                  <div className="relative z-10"><h4 className={`font-black text-2xl mb-2 transition-colors ${style.isPremium ? 'text-white' : 'text-slate-900 group-hover:text-theme'}`}>{style.name}</h4><p className={`text-sm font-medium leading-relaxed mb-6 ${style.isPremium ? 'text-slate-400' : 'text-slate-500'}`}>{style.description}</p></div>
                  <div className="mt-auto relative z-10 flex items-center gap-3"><div className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest flex items-center gap-2 transition-all ${style.isPremium ? 'bg-white/10 text-white group-hover:bg-white/20' : 'bg-slate-900 text-white group-hover:bg-theme'}`}>Generate {style.name.split(' ')[0]} <ArrowRight size={12} /></div></div>
            ...
            <div className="p-8 bg-white border-t border-slate-50 flex items-center justify-between gap-4 shrink-0">
              <div className="flex items-center gap-3"><Sparkle size={20} className="text-theme animate-spin duration-[4000ms]" /><span className="text-[10px] font-black uppercase tracking-[0.4em] text-slate-300">Synchronized with Gemini Hub</span></div>
        ...
        <div className="fixed inset-0 z-[120] flex items-center justify-center p-6 bg-slate-900/60 backdrop-blur-md animate-in fade-in">
          <div className="bg-white rounded-[2.5rem] w-full max-w-md shadow-2xl animate-in zoom-in-95 duration-300 overflow-hidden">
            <div className="p-8 bg-rose-50 border-b border-rose-100 flex items-center gap-4">
              ...
              <div><h3 className="text-xl font-black text-rose-900 tracking-tight">Purge Registry?</h3><p className="text-rose-700/80 font-medium text-xs">This cycle is irreversible.</p></div>
            </div>
            <div className="p-8 space-y-8">
              <div className="bg-slate-50 p-6 rounded-[1.5rem] border border-slate-100"><p className="text-slate-600 font-medium text-sm leading-relaxed text-center">Are you sure you want to permanently delete <strong className="text-slate-900 font-black">{selectedPet.name}</strong> from the guardian network?</p></div>
              <div className="flex gap-4">
                <button onClick={() => setShowDeleteModal(false)} disabled={isDeleting} className="flex-1 py-4 bg-slate-100 text-slate-500 rounded-xl font-black text-[11px] uppercase tracking-widest hover:bg-slate-200 transition-all active:scale-95">Abort</button>
                <button onClick={handleDeletePet} disabled={isDeleting} className="flex-1 py-4 bg-rose-500 text-white rounded-xl font-black text-[11px] uppercase tracking-widest hover:bg-rose-600 transition-all shadow-xl shadow-rose-500/20 active:scale-95 flex items-center justify-center gap-3 disabled:opacity-50">{isDeleting ? <Loader2 size={16} className="animate-spin" /> : <Trash2 size={16} />}Confirm Purge</button>
              </div>
            </div>
          </div>
        </div>
